const state = { bundle: null, module: 'order', basis: 'rmb', customer: null, lineKey: null, documentSignature: null };
...
function updateContext({ render = true } = {}) {
  ...
  document.body.dataset.basis = state.basis;
  document.querySelectorAll('[data-module]').forEach(button => button.classList.toggle('active', button.dataset.module === state.module));
  document.querySelectorAll('[data-source-module]').forEach(group => group.classList.toggle('active', group.dataset.sourceModule === state.module));
  document.querySelectorAll('[data-source-module] input[type="file"]').forEach(input => {
...
document.querySelectorAll('input[name="basis"]').forEach(input => {
...
function clearDragStates() {
  document.querySelectorAll('.drop-card.drag-over').forEach(card => card.classList.remove('drag-over'));
}
...
window.addEventListener('dragend', clearDragStates);
...
document.querySelectorAll('input[type="file"]').forEach(input => {
  ...
  input.addEventListener('change', () => updateFileCard(input));
...
$('#uploadForm').addEventListener('submit', async event => {
  ...
  try {
    ...
    formData.set('module', state.module);
    const documentFile = event.currentTarget.elements.document.files[0];
    const signature = documentFile ? `${documentFile.name}|${documentFile.size}|${documentFile.lastModified}` : '';
    ...
    state.bundle = {
      ...state.bundle,
      ...data,
      modules: { ...(state.bundle?.modules || {}), ...(data.modules || {}) },
    };
    ...
    showError(error.message || '网络异常，请重试。');
  } finally {
...
function renderResults({ scroll = false } = {}) {
  ...
  $('#downloadButton').onclick = () => { window.location.href = data.download_url; };
...
function renderCustomers() {
  ...
  document.querySelectorAll('[data-customer]').forEach(button => button.addEventListener('click', () => selectCustomer(button.dataset.customer)));
}
...
function selectCustomer(customer) {
  ...
  document.querySelectorAll('[data-key]').forEach(button => button.addEventListener('click', () => selectLine(button.dataset.key)));
